...
(C-Alpha)'.format(fragment_size,position),
            ...
            elif line.startswith('CA_RMSD'):
                tokens = line.rsplit(None, 1)
                meta = parse_extra_metric(tokens[0], 6)
                record[meta.name] = float(tokens[1])
                metadata[meta.name] = meta

            elif line.startswith('AllAtom_RMSD'):
                tokens = line.rsplit(None, 1)
                meta = parse_extra_metric(tokens[0], 7)
                record[meta.name] = float(tokens[1])
                metadata[meta.name] = meta
        ...
        for metric, values in list(restraint_values.items()):
            meta = ScoreMetadata(
        ...
        for metric, values_by_residue in list(restraint_values_by_residue.items()):
...
def find_pareto_front(metrics, metadata, columns, depth=1, epsilon=None, progress=None):
    ...
    labeled_metrics['_pip_index'] = list(range(len(metrics)))
...
class AtomPairRestraint(object):
    ...
    def __init__(self, args):
        ...
        self.atom_pair = list(zip(self.atom_names, self.residue_ids))
...
class DihedralRestraint(object):
    ...
    def __init__(self, args):
        ...
        self.atoms = list(zip(self.atom_names, self.residue_ids))
