...
Usage:
    roseasy make_table <workspaces>... [options]
...
Options:
    ...
    -l, --design-list LIST  
        Explicitely list designs to be written to table.
    ...
    -p, --prefix PREFIX
        ...
        This is useful for discriminating files generated by different
        runs.
...
from roseasy import pipeline, structures
...
class StructureClusterMetric (Metric):
    ...
    def read_loop_coords(self, design):
        ...
        for start, stop in design.loops:
            loop_indices += list(range(start, stop + 1))
...
def find_reasonable_designs(workspaces, condition=None, verbose=False):
    ...
    for workspace in workspaces:
        if workspace.subdirs:
            ...
            all_designs = structures.Design(workspace.output_dir)
            for structure in all_designs.structures.iterrows():
                designs.append(structures.SimpleDesign(structure,
                    all_designs.metadata, workspace.output_dir))
...
def discover_extra_metrics(metrics, designs):
    ...
    for key, metadata in sorted(list(extra_metrics.items()), key=sort_by_order):
...
def main():
    ...
    if args['--design-list']:
        designs = designs_from_list(workspaces, args['--design-list'],
